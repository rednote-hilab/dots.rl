...
from .pipeline_utils import PIPELINE_START_SINGLE, AsyncPipeline, TransferMode, enhanced_print
...
class BaseRoleStateMachine(ABC):
    ...
    def _on_state_change(self, old_state: RoleState, new_state: RoleState, event: RoleEvent):
        ...
        enhanced_print(
            self.role_name, self.role_name, f"Event: {event.name} (State: {old_state.name} -> {new_state.name})"
        )
...
class AsyncTrainingFlow:
    ...
    def __init__(self, trainer=None, use_async_rl=True):
        ...
        if use_async_rl:
            ...
            enhanced_print("AsyncTrainingFlow", None, "Using async mode pipeline with compression")
    ...
    def _create_state_machines(self):
        ...
        if self.use_async_rl:
            enhanced_print(
                "AsyncTrainingFlow", None, "Using async RL state machines for dual buffer and async param sync"
            )
        else:
            enhanced_print("AsyncTrainingFlow", None, "Using async state machines (legacy mode - not recommended)")

        # Import state machine implementation
        from .state_machine_impl import create_role_state_machine

        return {
            role_name: create_role_state_machine(role_name, self.pipeline, self.trainer, use_async_rl=self.use_async_rl)
            for role_name in [
                "dataloader",
                "rollout",
                "train",
    ...
    async def _init_before_pipeline(self):
        ...
        if self.trainer.config.trainer.get("share_resource_between_train_logp_ref_logp", True):
            ...
            enhanced_print(
                "train", None, f"Sent training completion signal for step {self.trainer.global_steps} to logp/ref_logp"
            )
