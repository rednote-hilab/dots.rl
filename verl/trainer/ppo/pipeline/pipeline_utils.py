...
def enhanced_print(src_role, dst_role, message):
    ...
    print(f"[{timestamp}] {src_color}[{src_role_formatted}]{Style.RESET_ALL} {message}")
...
class AsyncPipeline:
    ...
    def __init__(self, max_queue_size=1, transfer_mode=TransferMode.RAY_QUEUE_OPTIMIZED):
        ...
        enhanced_print(
            "pipeline",
            None,
            f"AsyncPipeline initialized: max_queue_size={max_queue_size}, transfer_mode={transfer_mode.value}",
        )

        # Initialize based on transfer mode
        if transfer_mode in [
            TransferMode.RAY_QUEUE,
            TransferMode.RAY_QUEUE_COMPRESSED,
            TransferMode.RAY_QUEUE_OPTIMIZED,
        ]:
            # Use Ray queue
    ...
    def register_queue(self, src_role, dst_role):
        ...
        if self.transfer_mode in [
            ...
            if use_queue_name not in self._queue_pairs:
                ...
                enhanced_print(
                    "pipeline", None, f"Registered queue: {use_queue_name} with maxsize={self.max_queue_size}"
                )
    ...
    def is_complete(self, src_role, dst_role):
        ...
        def _get_keys_before_cur_queue():
            ...
            for key in self._queue_pairs.keys():
                if key == first_queue_name:
                    continue
                if key == last_queue_name:
                    continue
                if key == use_queue_name:
                    break
    ...
    async def push(self, src_role, dst_role, data, debug_log=False):
        ...
        if self.transfer_mode == TransferMode.DIRECT_OBJECT_STORE:
            ...
            # Use Ray queue
            cur_pipeline_queue = self._queue_pairs.get(use_queue_name, None)
            assert cur_pipeline_queue is not None, (
                f"Queue {use_queue_name} not found in pipeline queues: {self._queue_pairs.keys()}"
            )
        ...
        if self.transfer_mode == TransferMode.DIRECT_OBJECT_STORE:
            ...
            if debug_log and total_time > 1.0:
                enhanced_print(
                    dst_role,
                    src_role,
                    f"⚠️ SLOW DIRECT GET: {use_queue_name} took {total_time:.2f}s (ray_get: {ray_get_time:.2f}s)",
                )
        else:
            # Use Ray queue
            cur_pipeline_queue = self._queue_pairs.get(use_queue_name, None)
