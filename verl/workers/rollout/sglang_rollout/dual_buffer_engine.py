...
class _BufferManager:
    ...
    def __init__(self, engine, bucket_size_mb=None, memory_efficient_mode=False):
        ...
        self._use_batch_serialize = (
            True  # Enable batch serialization: fresh serialize the whole list at once when switching
        )
        self._use_reqinput_prefetch = (
            False  # Prefetch serialization in recv stage, can further overlap serialization time; WIP
        )
        ...
        if self._memory_efficient_mode:
            ...
            enhanced_print(
                "BufferManager", None, "Memory efficient mode enabled: using single buffer to reduce CPU memory usage"
            )
        else:
        ...
        enhanced_print(
            "BufferManager", None, f"Initialized with bucket_size_mb={self._batch_size_mb}, tp_size={self.tp_size}"
        )
    ...
    def _ensure_buffer_dict(self, buf_idx: int) -> None:
        ...
        if buf_idx < 0 or buf_idx >= len(self._e._buffer_weights):
            enhanced_print(
                ...
                None,
                f"Invalid buffer index: {buf_idx}, valid range: 0-{len(self._e._buffer_weights) - 1}",
            )
            return
    ...
    def register_update(self, named_tensors, version: int, group_tensor_count: int) -> bool:
        ...
        if existing_version is not None and version < existing_version:
            enhanced_print(
                "DualBufferAsyncEngine",
                None,
                f"Ignore outdated update: buffer {target_buffer} has version {existing_version} > incoming {version}",
            )
            return False
        ...
        if self._use_batch_serialize:
            ...
            for (name, tensor), (s_name, s_list) in zip(iterable, serialized_items, strict=False):
                assert name == s_name
                self._e._buffer_weights[target_buffer][name] = tensor.detach().cpu()
        ...
        enhanced_print(
            "DualBufferAsyncEngine",
            None,
            f"Registered update: buffer {target_buffer}, version {version}, tensors {len(iterable)}",
        )
        return True
    ...
    def __new__(cls, **kwargs):
        ...
        class DualBufferAsyncEngineImpl(AsyncEngine):
            def __init__(self, **kwargs):
                ...
                if not self.enable_param_async:
                    ...
                    enhanced_print(
                        "DualBufferAsyncEngine", None, "NCCL GPU sync mode: using simplified direct load implementation"
                    )
                    return
                ...
                if self._memory_efficient_mode:
                    ...
                    enhanced_print(
                        "DualBufferAsyncEngine",
                        None,
                        "Memory efficient mode enabled: using single buffer to reduce CPU memory usage",
                    )
                else:
                    # Dual buffer mode: maintain two buffers for better performance
        ...
        def _apply_weights_to_engine_sync(self, weights, update_weights_func_call):
            ...
            if not self.enable_param_async:
                ...
                result = self._run_async_in_sync_context(update_weights_func_call(weights))
            else:
                # CPU async mode: use buffer management settings
                result = self._run_async_in_sync_context(
                    update_weights_func_call(weights, use_reqinput=self._bufman._use_reqinput)
                )
            ...
            if not self.enable_param_async:
                ...
                with self._update_lock:
                    enhanced_print(
                        "DualBufferAsyncEngine",
                        None,
                        f"Current version: {self._current_version}, Latest version: {self._latest_version}",
                    )
                    ...
                    if self._latest_version >= self._current_version:
                        ...
                        enhanced_print(
                            "DualBufferAsyncEngine",
                            None,
                            f"No new version available, current version: {self._current_version}",
                        )
                        return True
        ...
        def switch_to_buffer_sync(self, buffer_id=-1, update_weights_func_call=None):
            ...
            with self._update_lock:
                ...
                enhanced_print(
                    "DualBufferAsyncEngine",
                    None,
                    f"Build payload for buffer {buffer_id} took {t2 - t1:.2f} s, apply weights took {t3 - t2:.2f} s",
                )
                ...
                if success:
                    ...
                    enhanced_print(
                        "DualBufferAsyncEngine",
                        None,
                        f"Successfully applied and switched to buffer {buffer_id} for version {self._current_version}",
                    )
                    return True
                else:
                    enhanced_print(
                        "DualBufferAsyncEngine",
                        None,
                        f"Failed to apply weights from buffer {buffer_id} to engine; not switching",
                    )
                    return False
            ...
            if not self.enable_param_async:
                ...
                try:
                    ...
                    if hasattr(self.sharding_manager, "update_weights_sync"):
                        ...
                        enhanced_print(
                            "DualBufferAsyncEngine", None, f"Used sync update_weights for {len(tensor_list)} tensors"
                        )
                        return result
                    else:
                        # Fallback to async method with sync wrapper
                        result = self._run_async_in_sync_context(self.sharding_manager.update_weights(tensor_list))
                        return result
